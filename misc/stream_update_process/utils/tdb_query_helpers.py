...
def getResults(ans):
    results = [list(zip(a.keys(), [(i.get_type().get_label().name(), i.get_value())
                    for i in a.values() if i.is_attribute()])) for a in ans]
    results = [(k, convertDateUTC(v) if isinstance(v, datetime) else v)
               for k, v in results]
    return results
...
def deleteRelationQuery(rtype, nodes, identifiers, attr_type_dict):
    # nodes = [(etype, ids)]
    q = "match "
    q += ' '.join([f"$x{i} isa {etype} {formatAttrDB(ids , attr_type_dict)};" for i,
                  (etype, ids) in enumerate(nodes)])
    q += f"$rel "
    if nodes:
        q += f"({','.join([f'$x{i}' for i in range(len(nodes))])})"
...
def getRelationDB(rtype, ids, nodes, attr_type_dict, session,
                  date_indicator="PublishedOn", check=False):
    ...
    if not check:
        ...
        # grab latest one
        links = [getUniqueResult(i.concept_maps())
                 for i in matchgroupquery(query, session)]
        if links:
            max_date = max([dict(l).get(date_indicator, 0) for l in links])
            latest = [l for l in links if dict(l).get(
                date_indicator) == max_date][0] if max_date != 0 else links[0]
            return latest
    else:
        query += "; get $p; group $p;"
        links = [getUniqueResult(i.concept_maps())
...
def format_attr(v, atype):
    if (atype == 'string'):
        return f'"{re.sub(spc, " ", str(v)).lower()}"'
    # elif (atype=='datetime'): return to_datetime(v).strftime('%Y-%m-%dT%H:%M:%S')
    elif (atype == 'datetime'):
        return datetime.strftime(parser.parse(v), '%Y-%m-%dT%H:%M:%S')
    elif (atype == 'long'):
        return int(float(v))
    else:
        return v
...
def chunker(iterable, chunksize):
    return (iterable[pos: pos + chunksize]
            for pos in range(0, len(iterable), chunksize))

...
def changedAttrs(old:dict, new:dict, attr_type_dict):
    def format(k, v, atype, ref):
        if (type(v)==list) : return False #all([match(k, i, atype)!=ref for i in v]) TODO modify to handle lists
        elif (atype=='datetime'): return datetime.strftime(parser.parse(v),'%Y-%m-%dT%H:%M:%S') != str(ref)
        else: return f'{re.sub(spc, " ", str(v)).lower()}' != str(ref).strip()
    return [(k, v) for k,v in new.items() if format(k,v,attr_type_dict[k], old.get(k, None))]
